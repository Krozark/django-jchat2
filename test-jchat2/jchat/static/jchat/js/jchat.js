...
var chat_last_message_receive = 0;
var chat_id = 0;
...
function chat_bind() {
    ...
    $("#submitForm").submit( function () {
        $.ajax({
            data: {'message' : $('#chatText').val(),'chat_room_id':chat_id,'chat_last_message_receive':chat_last_message_receive},
            dataType: 'json',
            type: 'post',
            url: '/jchat/send/'
        });
...
function get_messages() {
    $.ajax({
        type: 'POST',
        data: {'chat_last_message_receive':chat_last_message_receive,'chat_room_id': chat_id},
        url:  '/jchat/receive/',
		dataType: 'json',
		success: function (json) {
            $.each(json, function(i,m){
                renderLine(m);
                chat_last_message_receive = m.id;
...
function chat_join() {
	$.ajax({
		async: false,
        type: 'POST',
        data: {'chat_room_id': chat_id,'chat_last_message_receive':chat_last_message_receive},
        url:'/jchat/join/',
        dataType : 'json',
        success : function(json){
            chat_last_message_receive = json.chat_last_message_receive;
            run = true;
            setTimeout("get_messages()", 2000);
        },
        error : function(){
